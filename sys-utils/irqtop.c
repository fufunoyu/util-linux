...
static struct colinfo infos[] = {
	[COL_IRQ] = {"IRQ", 0.20, SCOLS_FL_RIGHT, N_("interrupts")},
	[COL_COUNT] = {"COUNT", 0.20, SCOLS_FL_RIGHT, N_("total count")},
	[COL_DESC] = {"DESC", 0.60, SCOLS_FL_TRUNC, N_("description")},
};
...
static inline struct colinfo *get_column_info(const struct irqtop_ctl *ctl,
					      unsigned num)
{
	return &infos[get_column_id(ctl, num)];
}

static void add_scols_line(struct irqtop_ctl *ctl,
...
static int init_scols_table(struct irqtop_ctl *ctl)
{
	...
	scols_table_enable_raw(ctl->table, ctl->raw);
	...
	for (i = 0; i < ctl->ncolumns; i++) {
		...
		cl = scols_table_new_column(ctl->table, col->name, col->whint,
					    flags);
		if (cl == NULL) {
...
static void parse_args(struct irqtop_ctl *ctl, int argc, char **argv)
{
	...
	while ((o = getopt_long(argc, argv, "d:os:hV", longopts, NULL)) != -1) {
		...
		case COLUMNS_OPT:
			if (optarg) {
				ssize_t nc = string_to_idarray(optarg,
							       ctl->columns,
